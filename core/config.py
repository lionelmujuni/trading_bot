...
# Exchange Selection
# Set EXCHANGE=kraken or EXCHANGE=robinhood in your .env file.
# The get_exchange_client() factory below returns the correct client.
EXCHANGE = os.getenv("EXCHANGE", "kraken").lower()

# Robinhood API credentials (Ed25519)
API_KEY = os.getenv("API_KEY", "")
BASE64_PRIVATE_KEY = os.getenv("BASE64_PRIVATE_KEY", "")
BASE_URL = "https://trading.robinhood.com"

# Kraken API credentials (HMAC-SHA512)
...
def _ensure_env_secret(env_var: str, generator_fn) -> str:
    """Return env var value; auto-generate and persist to .env if absent."""
    ...
    try:
        with open(_env_file, "a") as _f:
            _f.write(f"\n{env_var}={val}\n")
        os.environ[env_var] = val
    except OSError:
        pass  # read-only filesystem — set the env var manually
    return val


# JWT secret — auto-generated on first run; stored in .env
JWT_SECRET_KEY: str = _ensure_env_secret("JWT_SECRET_KEY", lambda: _secrets.token_hex(32))

# Fernet symmetric key for credential encryption.
# IMPORTANT: Loss of this key = unrecoverable encrypted credentials. Back it up!
try:
    from cryptography.fernet import Fernet as _Fernet
    CREDENTIAL_ENCRYPTION_KEY: str = _ensure_env_secret(
        "CREDENTIAL_ENCRYPTION_KEY",
        lambda: _Fernet.generate_key().decode(),
    )
except ImportError:
    CREDENTIAL_ENCRYPTION_KEY: str = os.getenv("CREDENTIAL_ENCRYPTION_KEY", "")

# Google OAuth 2.0 client ID — set GOOGLE_CLIENT_ID=<your-id> in .env to enable Google login
...
def get_exchange_client(db=None):
    """
    Returns an exchange client for the configured EXCHANGE.
    Both clients implement the same public interface:
        get_account(), get_current_price(), get_best_bid_ask(),
        get_trading_pairs(), place_order(), cancel_order(),
        get_order(), get_orders(), poll_order_status()

    Args:
        db: Optional Database instance. If None, each client creates its own.
            Pass an existing instance to share the same nonce_counter connection.
    """
    if EXCHANGE == "kraken":
        ...
        from exchanges.robinhood_client import RobinhoodClient
        return RobinhoodClient(
            api_key=API_KEY,
            private_key_base64=BASE64_PRIVATE_KEY,
        )
    else:
        raise ValueError(
