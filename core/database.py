...
class Database:
    ...
    def init_database(self):
        ...
        with self.get_connection() as conn:
            ...
            # Table 10: Kraken Nonce Counter
            # Single-row table for strictly-increasing Kraken API nonces.
            # Never reset this value — nonces must always be larger than any previously used.
            cursor.execute("""
            ...
            """)
            # Seed with current ms timestamp if first run
            cursor.execute("""
                INSERT OR IGNORE INTO nonce_counter (id, value)
                VALUES (1, CAST(strftime('%s', 'now') AS INTEGER) * 1000)
            """)

            # Table 11: User-Defined Strategies
            # Stores strategy code written in the React Strategy Builder.
            # code is executed inside RestrictedPython sandbox for backtesting only.
            cursor.execute("""
            ...
            """)
            cursor.execute("""
                CREATE INDEX IF NOT EXISTS idx_user_strategies_name
                ON user_strategies(name)
            """)
            ...
            # Table 13: User Credentials (Fernet-encrypted exchange API keys)
            cursor.execute("""
    ...
    def get_next_nonce(self) -> int:
        """
        Atomically increment and return the next Kraken nonce.
        Guaranteed to be strictly increasing across restarts.
        """
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
            ...
            """)
            cursor.execute("SELECT value FROM nonce_counter WHERE id = 1")
            return cursor.fetchone()[0]

    # ==================== User Strategy Methods ====================

    def save_user_strategy(self, name: str, code: str, description: str = "",
                           parameters: list = None, custom_metrics: list = None) -> int:
        """Insert or update a user strategy. Returns the row id."""
        import json as _json
        params_json = _json.dumps(parameters or [])
        metrics_json = _json.dumps(custom_metrics or [])
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO user_strategies (name, description, code, parameters, custom_metrics)
                VALUES (?, ?, ?, ?, ?)
            ...
            """, (name, description, code, params_json, metrics_json))
            cursor.execute("SELECT id FROM user_strategies WHERE name = ?", (name,))
            return cursor.fetchone()[0]

    def get_user_strategy(self, strategy_id: int) -> Optional[Dict]:
        """Retrieve a single user strategy by id."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM user_strategies WHERE id = ?", (strategy_id,))
            row = cursor.fetchone()
            return dict(row) if row else None

    def list_user_strategies(self) -> List[Dict]:
        """List all user strategies ordered by name."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM user_strategies ORDER BY name")
            return [dict(r) for r in cursor.fetchall()]

    def delete_user_strategy(self, strategy_id: int) -> bool:
        """Delete a user strategy. Returns True if a row was removed."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM user_strategies WHERE id = ?", (strategy_id,))
            return cursor.rowcount > 0

    def set_user_strategy_enabled(self, strategy_id: int, enabled: bool) -> None:
        """Toggle enabled state for a user strategy."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE user_strategies SET enabled = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
    ...
    def update_user_google_id(self, user_id: int, google_id: str) -> None:
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def save_user_credentials(self, user_id: int, exchange: str,
                               api_key_encrypted: str,
                               private_key_encrypted: str) -> None:
        """Upsert Fernet-encrypted exchange credentials for a user."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO user_credentials
                    (user_id, exchange, api_key_encrypted, private_key_encrypted)
                VALUES (?, ?, ?, ?)
    ...
    def get_user_credentials_raw(self, user_id: int, exchange: str) -> Optional[Dict]:
        """Return the encrypted credential row (api_key_encrypted / private_key_encrypted)."""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM user_credentials WHERE user_id = ? AND exchange = ?",
                (user_id, exchange),
            )
            row = cursor.fetchone()
            return dict(row) if row else None

    def delete_user_credentials(self, user_id: int, exchange: str) -> None:
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
